...
function Track() {
  ...
  const handlerTimeStamp = async (event) => {
    const result = await SupplyChain.methods.viewTimestamps(ID).call();
    settimestamp(result);
    setdateArray([
      timestamptoDate(result.RMSsupplyTimestamp),
      timestamptoDate(result.manufacturingTimestamp),
      timestamptoDate(result.distributionTimestamp),
      timestamptoDate(result.retailTimestamp),
      timestamptoDate(result.soldTimestamp),
    ]);
    settimestamp(result);
  };
  ...
  const getExpirationDate = async (event) => {
    const result = await SupplyChain.methods.Date(ID).call();
    const productionDate = new Date(result[1] * 1000); // Multiply by 1000 to convert seconds to milliseconds
    ...
    setExpiryDate(formattedProductionDate);
  };
  ...
  return (
    <div>
      ...
      {/* Display individual timestamps */}
      <p>RMSsupplyTimestamp: {dateArray[0]}</p>
      <p>Manufacturing Timestamp: {dateArray[1]}</p>
      <p>Distribution Timestamp: {dateArray[2]}</p>
      <p>Retail Timestamp: {dateArray[3]}</p>
      <p>Sold Timestamp: {dateArray[4]}</p>
